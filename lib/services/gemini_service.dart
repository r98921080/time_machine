...
只輸出清單，每行一個，不要編號或其他文字。''';
  ...
  Future<Map<String, List<String>>?> generateGoalTargets(
    ...
    final prompt = '''為「$category - $subCategory」生成三個難度層次的「單次/當日打卡標準」。

這是每天打卡用的，使用者今天做到這件事就能打卡，明天重新計算。
絕對不是累積型或長期型目標。

✅ 正確範例（「健身 - 跑步」）：
- mini: 跑步10分鐘或步行3000步
- advanced: 跑步30分鐘（3-5公里）
- elite: 跑步60分鐘（8公里以上）
...
規則：
- Mini = 5-15分鐘，今天最低標準
- Advanced = 20-45分鐘，今天正常目標
- Elite = 45-90分鐘，今天全力以赴
- 每個必須有具體數字（時間/次數/頁數/公里等）
- 動詞開頭，描述「今天做什麼」
...
⚠️ 重要：所有 mini/advanced/elite 都是「今天打一次卡要達成什麼」的標準，明天重新計算。
❌ 不能寫「連續X天」「本週X次」「每天堅持」等累積型描述。
✅ 要寫「今天做X分鐘」「今天完成X次」「今天讀X頁」等單次可完成的描述。

方案：
1. 激進版：比現有標準更高，mini/advanced/elite 數字都往上調
2. 穩健版：參考達成率$rateStr 微調，循序漸進
3. 輕量版：降低門檻，忙碌時也能打卡

JSON格式：
[
  {"name":"激進版","description":"方案核心精神（20字以內）","mini":"今天做X（有數字）","advanced":"今天做X（有數字）","elite":"今天做X（有數字）"},
  {"name":"穩健版","description":"方案核心精神（20字以內）","mini":"今天做X（有數字）","advanced":"今天做X（有數字）","elite":"今天做X（有數字）"},
  {"name":"輕量版","description":"方案核心精神（20字以內）","mini":"今天做X（有數字）","advanced":"今天做X（有數字）","elite":"今天做X（有數字）"}
]
只輸出JSON陣列。''';
    try {
      final res = await _gen([Content.text(prompt)]);
